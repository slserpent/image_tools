<?php
/*
 * Removes letterboxing from images with subdirectory traversal. False positives are possible with solid colors along image edges, e.g. dark interiors, backdrops, or clear sky, so is best used on only images with known letterboxing. Can account for artifacting from low-quality JPEG artifacts. However, only works for solid color backgrounds; cannot remove letterboxing where the background is a zoomed blurred copy of the image. Use ImageMagick >7.0.8 for better accuracy.
 */

//the text to append to filenames when saving with rename output mode
$rename_text = ".cropped";
//the subdirectory name when saving with copy output mode
$copy_dir = "cropped";
//percent of color variation that is still considered background. use higher values if not all letterboxing is removed.
$threshold_fuzz = 0.01;
//percent of color variation when detecting final edge of background. this is used to account for low quality JPEG artifacting. may need to increase for heavy artifacting.
$max_fuzz = 0.15;
//the maximum number of pixels that the final and rough detections can differ. 8 pixels is the default and also the JPEG block size. if artifacting is more complex, can increase.
$rough_pixel_diff = 8;
//the minimum JPEG quality (0-100) when saving the cropped image, regardless of what the input quality was
$min_jpg_quality = 75;

require_once "util.php";
require_once "../script_output/script_output.php";
set_time_limit(0);

//the script can run in either CLI or CGI
$options = ScriptOutput::get_params();
//init the output class
$output = new ScriptOutput([['title' => "DeLetterbox ImageMagick Script", 'wrap' => 80]]);

if (!can_iterate($options)) {
	$output->header("Options");
	$output->begin_section();
	$output->line("path: path to image directory");
	$output->line("output: how to output modified images, either 'rename', 'copy', or 'overwrite'. Rename places \$rename_text before image file extension in same directory. Copy puts image file in relative directory \$copy_dir with same filename. Default method is 'rename'.");
	$output->line("pattern: regex pattern for files to match. Default is a pattern matching *.jpg and *.png files.");
	$output->line("tag: optional EXIF keyword tag to filter on.");
	$output->line("traverse: boolean indicating whether or not to traverse subdirectories. Default is traverse true.");
	$output->end_section();
	exit();
}

//handle input variables
if (has_value($options['path'])) {
    $path = $options['path'];
    if (!file_exists($path)) die("Invalid path.");
} else die("No path specified.");
if (has_value($options['output'])) {
    $output_method = $options['output'];
    if (!preg_match('/^(rename|copy|overwrite)$/i', $output_method)) die("Invalid output method.");
} else $output_method = "rename";
if (has_value($options['pattern'])) {
	$pattern = $options['pattern'];
} else $pattern = '/\.(png|jpg)$/i';
if (has_value($options['tag'])) {
	$tag = $options['tag'];
}
if (has_value($options['traverse'])) {
	$traverse = (bool)$options['traverse'];
} else $traverse = true;

$output->header("Current Options");
$output->line("Path: " . $path);
$output->line("Subdirectory Traversal: " . strtoupper(var_export($traverse, true)));
$output->line("Output Method: " . strtoupper($output_method));
$output->line("Fuzz: " . round($threshold_fuzz * 100) . "%");

//https://legacy.imagemagick.org/discourse-server/viewtopic.php?t=35579
$can_pct_bg = false;
$ver = Imagick::getVersion();
preg_match('/ImageMagick ([0-9]+\.[0-9]+\.[0-9]+)/', $ver['versionString'], $ver);
if(version_compare($ver[1], "7.0.8") >= 0){
	$can_pct_bg = true;
}

//get all our image files using traversal util function
$images = traverse_directory($path, $pattern, $tag, $traverse);

$output->header("DeLetterboxing");

$total_images = count($images);
$output->line("Total Images: $total_images");
$count_cropped = 0;
$count_skipped = 0;

$output->begin_section();
$output->set_wrap(false);

$output->line("[Progress] Status: Input Image (Quality) -> Output Image (Quality). Input WxH - Output WxH = Cropped WxH");

//iterate over matches
foreach ($images as $i => $image_file) {
	try {
		$mod_time = filemtime($image_file);
		$access_time = fileatime($image_file);

		//load file into imagick
		$image = new Imagick();
		if ($can_pct_bg) $image->setOption("trim:percent-background", "95%");
		$image->readImage($image_file);
		$in_quality = $image->getImageCompressionQuality();
		$in_height = $image->getImageHeight();
		$in_width = $image->getImageWidth();
		$quantum_range = $image->getQuantumRange();

		$cropped_image = clone $image;

		$cropped_image->trimImage($threshold_fuzz * $quantum_range['quantumRangeLong']);

		$out_height = $cropped_image->getImageHeight();
		$out_width = $cropped_image->getImageWidth();

		//check for changes
		if ($in_height == $out_height && $in_width == $out_width) {
			$count_skipped++;
			continue;
		}

		//if not PNG or low-quality JPEG
		//artifacting creates rough edges in low-quality JPEGs
		if ($in_quality != 0 && $in_quality < 95 && $max_fuzz > $threshold_fuzz) {
			//do a rough crop to finish
			$rough_fuzz = $max_fuzz;
			do {
				$cropped_image = clone $image;
				$cropped_image->trimImage($max_fuzz * $quantum_range['quantumRangeLong']);

				$rough_height = $cropped_image->getImageHeight();
				$rough_width = $cropped_image->getImageWidth();

				//if we're within the pixel difference limits for the rough crop, then we're done
				if ((abs($out_width - $rough_width) <= $rough_pixel_diff) && (abs($out_width - $rough_width) <= $rough_pixel_diff)) break;
				//otherwise, make the fuzz lower and try again
				//decrease the fuzz by a third of the difference between max and min fuzz each time
				$rough_fuzz -= ($max_fuzz - $threshold_fuzz) / 3;
			} while ($rough_fuzz > $threshold_fuzz);

			$out_height = $rough_height;
			$out_width = $rough_width;
		}

		//choose what path to output the file as based on the output method option
		$path_parts = pathinfo($image_file);
		switch ($output_method) {
			case "copy":
				$target_dir = $path_parts['dirname'] . "\\$copy_dir";
				if (!file_exists($target_dir)) mkdir($target_dir);
				$target_filename = $target_dir . "\\" . $path_parts['basename'];
				break;
			case "overwrite":
				$target_filename = $image_file;
				break;
			case "rename":
			default:
				$target_filename = $path_parts['dirname'] . "\\" . $path_parts['filename'] . $rename_text . "." . $path_parts['extension'];
		}

		//should automatically set the image format to the input format
		$cropped_image->setImagePage($out_width, $out_height, 0, 0);
		$out_quality = max($in_quality, $min_jpg_quality);
		$cropped_image->setImageCompressionQuality($out_quality);
		$cropped_image->writeImage($target_filename);
		$image->clear();
		$cropped_image->clear();

		$count_cropped++;

		//copy file attributes over to new file
		if (touch($target_filename, $mod_time, $access_time) === false) {
			$output->line("Failed to copy file times for $image_file.");
		} else {
			$output->line(sprintf("[%d%%] Success: %s (%d%%) -> %s (%d%%). %dx%d - %dx%d = %dx%d", round(($i/$total_images)*100), $image_file, $in_quality, $target_filename, $out_quality, $in_width, $in_height, $out_width, $out_height, -abs($in_width-$out_width), -abs($in_height-$out_height)));
		}
	} catch (Exception $ex) {
		$output->line("Failure: " . $ex->getMessage());
	}
}
$output->end_section();

$output->begin_section();
$output->line("Images Cropped: $count_cropped");
$output->line("Total Skipped: $count_skipped");

if (has_value($tag)) {
	$output->line("Remember to remove tag. It is not possible with this program.");
}